  ...
  return (
    <div id="tablero-ejecucion" className="scroll-mt-6">
      <Card className="overflow-hidden border-white/80 bg-white/72 p-0">
        ...
        {statuses.map((status) => {
          const count = selectedTasks.filter((task) => task.effectiveStatus === status).length;
      ...
      <div className="grid gap-4 md:grid-cols-[minmax(16rem,1.5fr)_minmax(10rem,0.8fr)_minmax(10rem,0.9fr)_minmax(9rem,0.75fr)] md:items-start">
        <div>
          <div className="flex flex-wrap items-center gap-2">
            ...
            <Badge tone={status.tone} className="md:hidden">{task.effectiveStatus}</Badge>
          </div>
          ...
          {task.totalSubtasks > 0 ? (
            <div className="mt-3 rounded-xl bg-white/65 p-3 ring-1 ring-white/80">
              <div className="mb-2 flex items-center justify-between gap-3 text-xs font-semibold text-slate-600">
                <span>{task.completedSubtasks}/{task.totalSubtasks} subtareas</span>
        ...
        <div>
          <TaskStatusForm taskId={task.id} currentStatus={task.effectiveStatus} canEdit={canEdit && task.totalSubtasks === 0} returnPhase={activePhase} className="mt-0" helper={task.totalSubtasks > 0 ? "Automatico por subtareas" : undefined} />
        </div>
